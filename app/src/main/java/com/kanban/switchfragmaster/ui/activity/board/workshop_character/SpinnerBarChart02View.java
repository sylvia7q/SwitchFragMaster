 ...
 * @author XiongChuanLiang<br/>(xcl_168@aliyun.com)
 */
public class SpinnerBarChart02View extends DemoView {
	...
	private List<SevenusSmtWorkshopCharacterEntity> characterEntities;

	public SpinnerBarChart02View(Context context, int chartStyle, List<SevenusSmtWorkshopCharacterEntity> characterEntities) {
		super(context);
...
//			mChart.getDataAxis().setAxisSteps(20);
	...
	private void chartDataSet()
	{
		...
		for (int i = 0; i < characterEntities.size(); i++) {
			double rate1 = Double.parseDouble(characterEntities.get(i).getsHrowingRate1().toString());
			double rate2 = Double.parseDouble(characterEntities.get(i).getsHrowingRate2().toString());
			double rate3 = Double.parseDouble(characterEntities.get(i).getsHrowingRate3().toString());
			dataSeriesA.add(rate1);
			dataSeriesB.add(rate2);
			dataSeriesC.add(rate3);
		}
//		dataSeriesA.add(50d);
//		dataSeriesA.add(25d);
//		dataSeriesA.add(20d);
		BarData BarDataA = new BarData("1#抛料率",dataSeriesA,Color.rgb(73, 135, 218));
				

//		dataSeriesB.add(35d);
//		dataSeriesB.add(65d);
//		dataSeriesB.add(75d);
		BarData BarDataB = new BarData("2#抛料率",dataSeriesB,Color.rgb(224, 4, 0));
		
//		List<Double> dataSeriesC= new LinkedList<Double>();
//		dataSeriesC.add(15d);
//		dataSeriesC.add(10d);
//		dataSeriesC.add(5d);
		BarData BarDataC = new BarData("3#抛料率",dataSeriesC,Color.rgb(255, 185, 0));
...
//		chartLabels.add("路人丙");
		if(characterEntities!=null){
			for (int i = 0; i < characterEntities.size(); i++) {
				chartLabels.add(characterEntities.get(i).getsLineNo());
			}
		}
	}
	//获取ArrayList中的最大值
	public double arrayListMax(){
		try{
			...
			double maxDevation2 = 0.0;
			int totalCount = characterEntities.size();
			if (totalCount >= 1){
				double maxRate1 = Double.parseDouble(characterEntities.get(0).getsHrowingRate1().toString());
				double maxRate2 = Double.parseDouble(characterEntities.get(0).getsHrowingRate2().toString());
				double maxRate3 = Double.parseDouble(characterEntities.get(0).getsHrowingRate3().toString());
				for (int i = 0; i < totalCount; i++){
					double tempRate1 = Double.parseDouble(characterEntities.get(i).getsHrowingRate1().toString());
					double tempRate2 = Double.parseDouble(characterEntities.get(i).getsHrowingRate2().toString());
					double tempRate3 = Double.parseDouble(characterEntities.get(i).getsHrowingRate3().toString());
					if (tempRate1 > maxRate1) {
						maxRate1 = tempRate1;
					}
					if (tempRate2 > maxRate2) {
						maxRate2 = tempRate2;
					}
					if (tempRate3 > maxRate3) {
						maxRate3 = tempRate3;
					}
				}
				maxDevation = maxRate1 > maxRate2 ? maxRate1:maxRate2;
				maxDevation2 = maxRate3 > maxDevation ? maxRate3:maxDevation;
			}
			return maxDevation2;
		} catch (Exception ex) {
			throw ex;
