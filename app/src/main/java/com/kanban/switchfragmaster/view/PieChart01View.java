...
public class PieChart01View extends DemoView implements Runnable{
	...
	public PieChart01View(Context context,String x1,String x2) {
		super(context);
	...
	private void chartRender()
	{
		try {					
			...
			float left = DensityUtil.dip2px(getContext(), 120);
			float top = DensityUtil.dip2px(getContext(), 20);
			float right = DensityUtil.dip2px(getContext(), 10);
			float bottom = DensityUtil.dip2px(getContext(), 10);
			...
			chart.getLabelPaint().setTextSize(15.0f);
			//标题
...
//			chart.showClikedFocus();
	...
	private void chartDataSet()
	{
		...
		//设置图表数据源		
//		chartData.add(new PieData("目标产量","20%",20,(int)Color.rgb(155, 187, 90)));
//		chartData.add(new PieData("实际产量","30%",30,(int)Color.rgb(191, 79, 75),false));
		chartData.add(new PieData("达成率" + " " + rate1+ "%",rate1 + "%",Double.valueOf(rate1),(int)Color.rgb(242, 167, 69)));
		//将此比例块突出显示
		chartData.add(new PieData("制令单达成率"+ " " + rate2+ "%",rate2 + "%",Double.valueOf(rate2),(int)Color.rgb(60, 173, 213),false));
...
//		chartData.add(new PieData("dispute","0%" ,  6,Color.rgb(90, 79, 88)));
...
//						chart.disablePanMode();
						...
						legend.setColSpan(5.0f);
						legend.setRowSpan(5.0f);
//						legend.setOffsetY(-55.0f);
						legend.setOffsetX(-5.0f);
